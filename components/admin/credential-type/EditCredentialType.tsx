...
import {editCredentialType, getCredentialTypeById} from "@/server/credential-type";

const EditCredentialType = (props: any) => {
    const {closeModal, loadData, dataId} = props

    const setIsLoading = useCommonStore((state:any) => state.setIsLoading);
    const [state, dispatch, isPending] = useActionState(editCredentialType, undefined);

    useEffect(() => {
        setIsLoading(true);
        const fetchData = async () => {
            try {
                const result = await getCredentialTypeById(dataId);
                console.log(result);
