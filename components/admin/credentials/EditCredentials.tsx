...
import {editCredential, getCredentialById} from "@/server/credential";
...
const EditCredentials = ({id}: { id: string }) => {
    ...
    const [state, dispatch, isPending] = useActionState(editCredential, undefined);
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                ...
                setValue('project', result?.data?.project_id);
                setValue('type', result?.data?.credential_type_id);
